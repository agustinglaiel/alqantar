...
function FutureUpgrade({ 
  ...
  const icons = {
    construction: (
      <svg className="size-full text-accent-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className={`flex flex-col items-center justify-center text-center ${currentSize.container} ${className}`}>
      ...
      <h1 className={`mb-3 font-bold text-ink-900 ${currentSize.title}`}>
        {title}
      </h1>

      {/* Mensaje */}
      <p className={`mb-6 max-w-md leading-relaxed text-ink-700 ${currentSize.message}`}>
        {message}
      </p>
      ...
      <div className="flex space-x-2">
        <div
          className={`${currentSize.dots} animate-pulse rounded-full bg-accent-600`}
          style={{ animationDelay: '0ms', animationDuration: '1.5s' }}
        />
        <div
          className={`${currentSize.dots} animate-pulse rounded-full bg-gold-500`}
          style={{ animationDelay: '300ms', animationDuration: '1.5s' }}
        />
        <div
          className={`${currentSize.dots} animate-pulse rounded-full bg-accent-700`}
          style={{ animationDelay: '600ms', animationDuration: '1.5s' }}
        />
      </div>

      {/* Texto adicional sutil */}
      <div className="mt-4 flex items-center space-x-1 text-xs text-ink-500">
